...
public class Package2 {
	...
	public static void main(String[] args) {
		...
		//Collection Framwork에서 데이터 타입을 지정할 땐 제너릭을 사용하여 지정, 참조형 데이터 타입만 지정가능
		List<Integer> list;
		...
		//add() : List에 요소를 추가
		//추가할 요소는 제너릭으로 지정한 데이터 타입의 요소이어야 한다.
		list.add(10);
//		list.add("99");
		...
		//List 인덱스 범의를 벗어나는 인덱스를 지정할 수 없다
		list.set(1, -100);
		System.out.println(list );
		...
		//set : 데이터를 저장할때 중복을 허용하지 않고 순서가 존재하지 않은 데이터 구조(집합)
		Set<String> set;
		
		//HashSet 구현 클래스 : 검색이 빠르다
		set = new HashSet<>();
		//TreeSet 구현 클래스 : 요솔ㄹ 정렬해서 추가
		set = new TreeSet<>();
		
		//add() : Set에 요소를 추 동일한 요소가 존재한다면 추가 불
		set.add("apple");
		...
		//존재하지 않는 키를 검색하면 null을 반
		System.out.println(map.get("apple"));
		
		//containsKey() : Map에 특정키가 존재하는지 확인
		System.out.println(map.containsKey("Apple"));
		
		//remove() : Map에서 특정 키를 가지는 키와 쌍을 제거
		//삭제에 성공하면 해당 키값을 값이 반환한다.
