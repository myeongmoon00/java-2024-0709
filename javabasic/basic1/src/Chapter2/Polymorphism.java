...
//다형성 : 하나의 표현으로 여러가지 형태를 나타낼수 있는것

//오버로딩 : 같은 클래스 내부에서 동일한 이름의 메서드를 여러개 정의할 수 있도록 하는것
//오버라이딩 : 자손 클래스에서 조상클래스(부모클래스)에 정의된 메서드를 재정의 하는 것

//업 캐스팅 : 하위 클래스의 객체를 상위 클래스 혹은 구현된 인터페이스 타입의 변수에 담을 수 있도록 하는 것
//다운 캐스팅 : '업 캐스팅'된 객체를 다시 원래 타입의 변수에 담을 수 있도록 하는 것
...
//		System.out.println(footballPlay.age);
		...
		FootballPlayer footFootballPlayer2 = (FootballPlayer) human2;
		System.out.println(footFootballPlayer2.name);
		System.out.println(footFootballPlayer2.age);
		System.out.println(footFootballPlayer2.team);
		footFootballPlayer2.kick();
		footFootballPlayer2.foul();
	...
	void method (Object object) {
		...
		//매개변수나 반환타입으로 object 타입을 지정하게 되면 해당
		//인스턴스(강제 다운캐스팅시 타입 불일치) 사용시 문제가할 수 있음
		//object;
	}
}
