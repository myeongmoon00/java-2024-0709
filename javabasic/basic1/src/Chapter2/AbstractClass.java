...
//추상 클래스 자체로 인스턴스를 생성할 수 없다. 반드시 추상클래스를 확장한 하위 클래스로 인스턴스를 생성해야 함 
//abstract class 추상클래스명{}

//추상 메서드 : 선언부만 존재하고 구현부는 존재하지 않음
//반드시 추상 클래스 내부또는 인터페이스에서 선언할 수 있음
//반드시 추상 클래스를 확장한 하위 클래스에서 재정의(오버라이딩)하여 구현
//abstract 반환타입 추상메서드(매개변수);
...
class Developer2 extends Human2{
		...
		void develop() {
			System.out.println(name + "이 " + Arrays.toString(skills) + "로 개발 합니다.");
		}
	}
...
class Police2 extends Human2{
	@Override
	void greet() {
		System.out.println("저는 "+ name + "이고 "+age +"세 이면서 경찰 입니다.");
	}
