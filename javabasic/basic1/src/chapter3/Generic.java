...
//Test4 형태로 클래스로 작성하면 코드의 중복은 해결할 수 있지만 
//사용시 타입의 안정성이 떨어집니다.
class Test4 {
	String subject;
	Object score;
...
class ExtendsClass<T extends MyClass> { T Variable; }
...
public class Generic {
	...
	//?super 클래스 : 특정 클래스 혹은 특정클래스의 상위를 상속받은 클래스만 타입으로 받을수 있도록 제
	...
	public static void main(String[] args) {
		...
		//T extends 클래스명 : 특정 클래스 혹은 특정 클래스를 상속한 클래스만 제너릭 타입으로 받을 수 있도록 제한함
//		ExtendsClass<MyClass> instance3 = new ExtendsClass<>();
		
		//? : 제너릭의 와일드 카드로 모든 타입을 받을 수 있도록 한다.
		//와일드카드가 사용되는 제너릭은 메서드의 매개변수 타입이나, 반환타입에 사용됨
...
//		MyClass 혹은 MyClass의 부모만 받을수 있다 
		superMethod(list4);
		superMethod(list3);
//		superMethod(list5);
//		superMethod(list2);
